...
interface NavbarProps {
  ...
  // NEW PROP: Always show links on mobile
...
const LinkItem = ({ 
  ...
  dropdownArrow = true,
  isMobile = false,
  visibleLinks = false // Add visibleLinks prop here
}: { 
  link: NavLink;
  renderLink?: (link: NavLink, index: number) => React.ReactNode;
  linkPadding?: string;
  ...
  dropdownArrow?: boolean;
  isMobile?: boolean;
  visibleLinks?: boolean; // Add visibleLinks prop
}) => {
  ...
  return (
    ...
    >
      ...
      {/* Dropdown Menu */}
      {hasChildren && isOpen && (
        <div className={`nav_dropdown-menu ${(isMobile && !visibleLinks) ? 'nav_dropdown-mobile' : ''}`}> {/* Updated condition */}
          {link.children!.map((child, index) => (
            <LinkItem 
              ...
              isMobile={isMobile}
              visibleLinks={visibleLinks} // Pass visibleLinks down
            />
          ))}
        </div>
      )}
...
export default function AppBar(localProps: NavbarProps) {
      ...
      <div className="left-section" style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
        ...
        {hasLeftLinks && (!isMobileScreen || final.visibleLinks) && ( // Updated condition
          <NavLinks 
            ...
            visibleLinks={final.visibleLinks} // Pass visibleLinks
          />
        )}
      </div>
    );
  ...
  const renderMobileMenu = () => {
    if (!isMobileScreen || !isMobileMenuOpen || final.visibleLinks) return null; // Don't show mobile menu when visibleLinks is true
    ...
    <>
      <nav
        ...
        className={`navigation-bar
          ...
          ${final.visibleLinks ? 'visible-links-mode' : ''} // Add class for styling
        `}
        style={{
      ...
      >
        ...
        <div className={`linkWrapper ${final.visibleLinks ? 'navbar-links-visible' : ''}`}>
          {renderCenterSection()}
        </div>
        ...
        {/* Only show hamburger menu when visibleLinks is false and on mobile */}
        {isMobileScreen && !isMobileMenuOpen && !final.visibleLinks && (
          <>
            {final.hasSidebar ? 
